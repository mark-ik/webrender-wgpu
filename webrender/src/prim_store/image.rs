...
use api::{
    AlphaType, ColorDepth, ColorF, ColorU, ExternalImageType, ImageKey as ApiImageKey,
    ImageBufferKind, ImageRendering, PremultipliedColorF, RasterSpace, Shadow, YuvColorSpace,
    ColorRange, YuvFormat,
};
use api::units::*;
use euclid::point2;
...
use crate::prim_store::{
    EdgeAaSegmentMask, PrimitiveInstanceKind, PrimitiveOpacity, PrimKey, PrimTemplate,
    PrimTemplateCommonData, PrimitiveStore, SegmentInstanceIndex, SizeKey, InternablePrimitive,
};
        ...
        match image_properties {
            ...
            Some(ImageProperties {
                ...
                let mut size = frame_state
                    .resource_cache
                    .request_image(request, frame_state.gpu_cache);
                ...
                if self.tile_spacing == LayoutSize::zero() {
                    ...
                    let padding = DeviceIntSideOffsets::new(
                        0,
                        (self.tile_spacing.width * size.width as f32 / self.stretch_size.width)
                            as i32,
                        (self.tile_spacing.height * size.height as f32 / self.stretch_size.height)
                            as i32,
                        0,
                    );
            ...
            Some(ImageProperties {
                ...
                let repetitions =
                    image_tiling::repetitions(&common.prim_rect, &visible_rect, stride);

                for image_tiling::Repetition { origin, edge_flags } in repetitions {
                    let edge_flags = base_edge_flags | edge_flags;

                    let layout_image_rect =
                        LayoutRect::from_origin_and_size(origin, self.stretch_size);
