...
use api::{
    FilterOp, FilterPrimitive, FontInstanceKey, FontSize, GlyphInstance, GlyphOptions, GradientStop,
};
...
use api::{
    ClipMode, PrimitiveKeyKind, TransformStyle, YuvColorSpace, ColorRange, YuvData, TempFilterData,
};
...
use crate::spatial_tree::{
    SceneSpatialTree, SpatialNodeContainer, SpatialNodeIndex, get_external_scroll_offset,
};
...
use crate::internal_types::{
    FastHashMap, LayoutPrimitiveInfo, Filter, FilterGraphNode, FilterGraphOp,
    FilterGraphPictureReference, PlaneSplitterIndex, PipelineInstanceId,
};
...
impl<'a> SceneBuilder<'a> {
    ...
    fn push_iframe(
        ...
        let bounds = self.normalize_scroll_offset_and_snap_rect(&info.bounds, spatial_node_index);
    ...
    fn process_common_properties(
        ...
        if common.flags.contains(PrimitiveFlags::ANTIALISED) {
            ...
            clip_rect = self.normalize_scroll_offset_and_snap_rect(&clip_rect, spatial_node_index);

            prim_rect = self.normalize_scroll_offset_and_snap_rect(&prim_rect, spatial_node_index);
        }
    ...
    fn build_item<'b>(&'b mut self, item: DisplayItemRef) {
        match *item.item() {
            ...
            DisplayItem::HitTest(ref info) => {
                ...
                let rect =
                    self.normalize_scroll_offset_and_snap_rect(&info.rect, spatial_node_index);
    ...
    fn add_image_mask_clip_node(
        ...
        let snapped_mask_rect =
            self.normalize_scroll_offset_and_snap_rect(&image_mask.rect, spatial_node_index);
    ...
    fn add_rect_clip_node(
        ...
        let snapped_clip_rect =
            self.normalize_scroll_offset_and_snap_rect(clip_rect, spatial_node_index);
    ...
    fn add_rounded_rect_clip_node(
        ...
        let snapped_region_rect =
            self.normalize_scroll_offset_and_snap_rect(&clip.rect, spatial_node_index);
