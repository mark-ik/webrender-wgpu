...
/// Walks `res/spirv/*.spv` and emits a `spirv_blobs.rs` file in OUT_DIR
/// with a static lookup function. Each `.spv` is embedded via
/// `include_bytes!`, so the resulting binary doesn't need runtime fs
/// access to the corpus. Only emitted when the wgpu_backend feature is
/// active (the corpus only matters there).
fn write_spirv_blobs(out_dir: &str) -> Result<(), std::io::Error> {
    let spirv_dir = Path::new("res/spirv");
    let out_path = Path::new(out_dir).join("spirv_blobs.rs");
    ...
    writeln!(
        out,
        "/// Lookup committed SPIR-V blob by (stem, stage). Returns None if"
    )?;
    writeln!(
        out,
        "/// no matching artifact was committed at build time."
    )?;
    writeln!(out, "pub fn lookup(stem: &str, stage: &str) -> Option<&'static [u8]> {{")?;
    writeln!(out, "    match (stem, stage) {{")?;

    if spirv_dir.exists() {
        let mut entries: Vec<_> = read_dir(spirv_dir)?
            .filter_map(|e| e.ok())
            .map(|e| e.path())
        ...
        for path in entries {
            ...
            // Names: "{stem}.{stage}.spv"
            let without_spv = file_name.trim_end_matches(".spv");
            if let Some((stem, stage)) = without_spv.rsplit_once('.') {
                let escaped = escape_include_path(&path);
...
fn main() -> Result<(), std::io::Error> {
    ...
    // Always emit the SPIR-V blob lookup. When wgpu_backend is off the
    // generated module is unused (cargo just discards it); when on, the
    // shaders module includes it via `include!(concat!(env!("OUT_DIR"),
    // "/spirv_blobs.rs"))`.
    write_spirv_blobs(&out_dir)?;
