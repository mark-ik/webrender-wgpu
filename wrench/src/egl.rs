...
impl Context {
    ...
    pub fn new<'a>(
        ...
        let (config_id, pixel_format) =
            unsafe { choose_fbconfig(display, &egl_version, api, version, pf_reqs)? };
        ...
        let context = unsafe {
            if let Some(version) = self.version {
                ...
                if let Ok(ctxt) = create_context(
                    ...
                    ctxt
                } else if let Ok(ctxt) = create_context(
                    self.display,
                    &self.egl_version,
                    &self.extensions,
                    self.api,
