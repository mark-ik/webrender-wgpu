...
//! `BeginRenderPass` per target switch.

use std::ops::Range;

/// Colour attachment policy for one wgpu render pass. The load
/// operation is declared when the pass begins, not as mutable device
/// state.
...
/// Depth attachment policy for one wgpu render pass. Load and store
/// behavior are declared with the pass, not patched as mutable device
/// state afterward.
...
/// Target description for a single wgpu render pass. Renderer callsites
/// construct this from render-task targets instead of binding GL FBO
/// state on the device.
...
/// inside a single render-pass scope.
///
...
pub struct DrawIntent {
    ...
    /// `has_dynamic_offset: true`. The length of this slice must match
    /// the count of dynamic-offset entries in the bind group's layout.
    /// Empty if the bind group has no dynamic offsets.
    pub dynamic_offsets: Vec<u32>,
    /// Push-constant payload; stage VERTEX. Empty if the pipeline has
